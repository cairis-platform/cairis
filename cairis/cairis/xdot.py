    ...
    def draw(self, cr, highlight=False,zoom_ratio=-1):
        if (len(self.shapes) == 1) and (self.shapes[0].__class__.__name__ == 'BezierShape') and (self.url != '') and (zoom_ratio >= armid.LOW_ZOOM_RATIO and zoom_ratio < armid.HIGH_ZOOM_RATIO):
          ...
          try:
            ...
            pass
          if (src == 'role') or (dest == 'role') or (src == 'attacker') or (dest == 'attacker') or (src == 'persona') or (dest == 'persona') or (src == 'goal') or (dest == 'goal') or (src == 'obstacle') or (dest == 'obstacle') or (src == 'countermeasure') or (dest == 'countermeasure') or (src == 'response') or (dest == 'response') or (src == 'misusecase') or (dest =='misusecase'):
            return 
        for shape in self.shapes:
            shape.draw(cr, highlight=highlight,zoom_ratio=zoom_ratio)
...
class Graph(Shape):
    ...
    def draw(self, cr, highlight_items=None):
        ...
        for shape in self.shapes:
            shape.draw(cr,zoom_ratio=self.zoom_ratio)
        for edge in self.edges:
            edge.draw(cr, highlight=(edge in highlight_items),zoom_ratio=self.zoom_ratio)
        for node in self.nodes:
            node.draw(cr, highlight=(node in highlight_items),zoom_ratio=self.zoom_ratio)
...
class DotWidget(gtk.DrawingArea):
    ...
    def zoom_image(self, zoom_ratio, center=False, pos=None):
        ...
        self.graph.zoom_ratio = zoom_ratio
        self.zoom_to_fit_on_resize = False
        self.queue_draw()
