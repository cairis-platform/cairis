...
CREATE VIEW quotation as
   select c.name code,'internal_document',ind.name,'None' section,internalDocumentQuotationString(ind.name,idc.start_index,idc.end_index) quote from code c, internal_document ind, internal_document_code idc where c.id = idc.code_id and ind.id = idc.internal_document_id
   union
   select c.name code,'persona',p.name,'Activities' section,personaQuotationString(p.name,'activities',pc.start_index,pc.end_index) quote from code c, persona p, persona_code pc where c.id = pc.code_id and p.id = pc.persona_id
   union
   select c.name code,'persona',p.name,'Attitudes' section,personaQuotationString(p.name,'attitudes',pc.start_index,pc.end_index) quote from code c, persona p, persona_code pc where c.id = pc.code_id and p.id = pc.persona_id
   union
   select c.name code,'persona',p.name,'Aptitudes' section,personaQuotationString(p.name,'attitudes',pc.start_index,pc.end_index) quote from code c, persona p, persona_code pc where c.id = pc.code_id and p.id = pc.persona_id
   union
   select c.name code,'persona',p.name,'Motivations' section,personaQuotationString(p.name,'motivations',pc.start_index,pc.end_index) quote from code c, persona p, persona_code pc where c.id = pc.code_id and p.id = pc.persona_id
   union
   select c.name code,'persona',p.name,'Skills' section,personaQuotationString(p.name,'skills',pc.start_index,pc.end_index) quote from code c, persona p, persona_code pc where c.id = pc.code_id and p.id = pc.persona_id;
