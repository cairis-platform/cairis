#$URL: svn://edison.comlab.ox.ac.uk/res08/iris/iris/apxdot.py $ $Id: apxdot.py 330 2010-10-31 15:01:28Z shaf $
import xdot
...
class ATDotWindow(gtk.Window):
    ...
    def __init__(self):
        ...
        taskFrame = gtk.Frame()
        taskFrame.set_label("Task")
        taskFrame.set_shadow_type(gtk.SHADOW_ETCHED_IN)

        cBox.pack_start(taskFrame)
        self.taskCombo = gtk.ComboBoxEntry()
        taskFrame.add(self.taskCombo)
    ...
    def loadFilters(self,tasks=[],chars=[]):
      taskModel = gtk.ListStore(str)
      tIdx = 0
      taskModel.append([''])
      for tidx,task in enumerate(tasks):
        if (task == self.theInitialTask):
          tIdx = tidx + 1
          self.theTaskPersona = ''
        taskModel.append([task]) 
      self.taskCombo.set_model(taskModel)
      self.taskCombo.set_text_column(0)
      self.taskCombo.set_active(tIdx)
      self.taskHandlerId = self.taskCombo.connect('changed',self.onTaskChange)
    ...
    def onTaskChange(self, action):
      taskName = self.taskCombo.get_active_text()
      self.refreshModel(taskName,'')
    ...
    def onCharChange(self, action):
      ...
      self.refreshModel(taskName,charName)
    ...
    def refreshModel(self,taskName,charName):
      try:
        b = Borg()
        proxy = b.dbProxy
        associationDictionary = {}
        associations = proxy.assumptionTaskModel(taskName,charName)
        self.canonicalModel = AssumptionTaskModel(associations)

        self.set_xdotcode(self.canonicalModel.graph())
        self.widget.zoom_to_fit()
        self.set_title('Assumption Task Model')
    ...
    def on_print(self, action):
      ...
      if chooser.run() == gtk.RESPONSE_OK:
        ...
        if ((fileType != 'svg') and (fileType != 'pdf')):
            ...
            dlg.set_title('Assumption Task Model Viewer')
            dlg.run()
            dlg.destroy()
        else:
