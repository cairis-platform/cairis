#$URL: svn://edison.comlab.ox.ac.uk/res08/iris/iris/envxdot.py $ $Id: envxdot.py 578 2012-03-18 23:08:59Z shaf $
import xdot
...
REMOTE_COLOUR = (0.9922,0.7333,0.5176)
OCCASIONAL_COLOUR = (0.9882,0.5529,0.3490)
PROBABLE_COLOUR = (0.8902,0.2902,0.2000)
FREQUENT_COLOUR = (0.7020,0,0)

NEGLIGIBLE_COLOUR = (0.9961,0.9412,0.8510) 
MARGINAL_COLOUR = (0.9922,0.7333,0.5176)
...
class EnvironmentTextShape(xdot.TextShape):
    ...
    def __init__(self, pen, x, y, j, w, t,dim):
        ...
        if (dim == 'asset' or dim == 'threat' or dim == 'vulnerability'):
#          self.y = y - 5
          self.y = y - 15
        elif (dim == 'requirement'):
          self.y = y + 22
        elif (dim == 'attacker' or dim == 'persona'):
          self.y = y + 22
        elif (dim == 'role'):
          self.y = y + 20
...
class TaskShape(xdot.EllipseShape):
    def __init__(self, pen, x0, y0, w, h, filled=False):
...
class XDotAttrParser(xdot.XDotAttrParser):
    def __init__(self, parser, buf,dim='',objt='',environment='',dp=''):
    ...
    def handle_text(self,x,y,j,w,t):
      self.shapes.append(EnvironmentTextShape(self.pen, x, y, j, w, t,self.dim))
    
    def handle_ellipse(self, x0, y0, w, h, filled=False):
      if (self.dim == 'requirement'):
        if filled:
    ...
    def handle_polygon(self,points,filled=False):
      if filled:
        self.shapes.append(EnvironmentPolygonShape(self.pen,points,self.dim, self.objt,self.environment,self.dbProxy,filled=True))
      self.shapes.append(EnvironmentPolygonShape(self.pen,points,self.dim, self.objt,self.environment,self.dbProxy))

class EnvironmentXDotParser(xdot.XDotParser):

    def __init__(self, xdotcode,environment,dp):
    ...
    def handle_node(self, id, attrs):
        ...
        for attr in ("_draw_", "_ldraw_"):
            if attr in attrs:
                ...
                parser = XDotAttrParser(self, attrs[attr],dimObjt[0],dimObjt[1],self.environment,self.dbProxy)
...
class EnvironmentDotWidget(xdot.DotWidget):
  def __init__(self):
    xdot.DotWidget.__init__(self)

  def set_xdotcode(self, xdotcode,environment,dp):
    parser = EnvironmentXDotParser(xdotcode,environment,dp)
...
class EnvironmentDotWindow(gtk.Window):
    ...
    def __init__(self,windowTitle,dp):
        ...
        self.widget = EnvironmentDotWidget()
        ...
        typeFrame = gtk.Frame()
        typeFrame.set_label("Artifact")
        typeFrame.set_shadow_type(gtk.SHADOW_ETCHED_IN)
        hbox.pack_start(typeFrame)
        self.typeCombo = gtk.ComboBoxEntry()
        typeFrame.add(self.typeCombo)

        nameFrame = gtk.Frame()
        nameFrame.set_label("Name")
        nameFrame.set_shadow_type(gtk.SHADOW_ETCHED_IN)
        hbox.pack_start(nameFrame)
        self.nameCombo = gtk.ComboBoxEntry()
        nameFrame.add(self.nameCombo)
        # add end
    ...
    def loadFilters(self, environments, dimModel, nameModel):
      ...
      self.typeCombo.set_model(dimModel)
      self.typeCombo.set_text_column(0)
      self.nameCombo.set_model(nameModel)
      self.nameCombo.set_text_column(0)

      self.environmentHandlerId = self.environmentCombo.connect('changed',self.onEnvironmentChange)
      self.typeHandlerId = self.typeCombo.connect('changed',self.onTypeChange)
      self.nameHandlerId = self.nameCombo.connect('changed',self.onNameChange)

    def blockHandlers(self):
      self.environmentCombo.handler_block(self.environmentHandlerId)
      self.typeCombo.handler_block(self.typeHandlerId)
      self.nameCombo.handler_block(self.nameHandlerId)

    def unblockHandlers(self):
      self.environmentCombo.handler_unblock(self.environmentHandlerId)
      self.typeCombo.handler_unblock(self.typeHandlerId)
      self.nameCombo.handler_unblock(self.nameHandlerId)
    ...
    def onTypeChange(self,action):
      self.blockHandlers()
      self.refreshModel()
      self.nameCombo.set_active(0)
      self.unblockHandlers()
    
    def onNameChange(self,action):
      self.blockHandlers()
      self.refreshModel()
      self.unblockHandlers()
    ...
    def onLayoutChange(self,action):
      ...
      self.set_xdotcode(self.traceModel.layout(renderer),self.environment,self.dbProxy)
    ...
    def set_xdotcode(self, xdotcode, environment = None, dp = None, filename='<stdin>'):
        if (environment == None):
          environment = self.environment
        if (dp == None):
          dp = self.dbProxy
        if self.widget.set_xdotcode(xdotcode,environment,dp):
            self.set_title(os.path.basename(filename) + ' - Dot Viewer')
            self.widget.zoom_to_fit()
    ...
    def refreshModel(self):
      ...
      if (self.typeCombo.get_active()):
        dimName = self.typeCombo.get_active_text()
      if (self.nameCombo.get_active()):
        objtName = self.nameCombo.get_active_text()

      try:
        b = Borg()
        proxy = b.dbProxy
        self.traceModel = EnvironmentModel(self.dbProxy.riskAnalysisModel(environmentName,dimName,objtName),environmentName,proxy)
        self.set_xdotcode(self.traceModel.graph())
        self.typeCombo.set_model(self.traceModel.dimensions())
        self.nameCombo.set_model(self.traceModel.objects())
        self.widget.zoom_to_fit()
