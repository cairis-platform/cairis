...
class SummaryPage(wx.Panel):
  def __init__(self,parent,dp):
    ...
    self.dependenciesCtrl = wx.TextCtrl(self,TASK_TEXTDEPENDENCIES_ID,'',size=(150,100),style= wx.TE_MULTILINE)
    ...
    self.personaList = TaskPersonaListCtrl(self,TASK_LISTPERSONAS_ID,self.dbProxy)
    ...
    self.assetList = DimensionListCtrl(self,TASK_LISTASSETS_ID,wx.DefaultSize,'Concerns','asset',self.dbProxy)
...
class NarrativePage(wx.Panel):
  def __init__(self,parent):
    ...
    self.narrativeCtrl = NarrativeCtrl(self,TASK_TEXTNARRATIVE_ID)
...
class TaskEnvironmentNotebook(wx.Notebook):
  def __init__(self,parent,dp):
    ...
    p3 = MLTextPage(self,TASK_TEXTCONSEQUENCES_ID)
    p4 = MLTextPage(self,TASK_TEXTBENEFITS_ID)
    p5 = ConcernAssociationPage(self,TASK_LISTCONCERNASSOCIATIONS_ID,dp)
