...
class NewEnvironmentDialog(wx.Dialog):
  def __init__(self,parent):
    ...
    mainSizer.Add(WidgetFactory.buildTextSizer(self,'Name',(87,30),NEWENVIRONMENT_TEXTNAME_ID),0,wx.EXPAND)
    mainSizer.Add(WidgetFactory.buildTextSizer(self,'Short Code',(87,30),NEWENVIRONMENT_TEXTSHORTCODE_ID),0,wx.EXPAND)
    mainSizer.Add(WidgetFactory.buildMLTextSizer(self,'Description',(87,30),NEWENVIRONMENT_TEXTDESCRIPTION_ID),1,wx.EXPAND)
    mainSizer.Add(WidgetFactory.buildCommitButtonSizer(self,NEWENVIRONMENT_BUTTONCOMMIT_ID,True))
    self.SetSizer(mainSizer)
    wx.EVT_BUTTON(self,NEWENVIRONMENT_BUTTONCOMMIT_ID,self.onCreate)


  def onCreate(self,evt):
    nameCtrl = self.FindWindowById(NEWENVIRONMENT_TEXTNAME_ID)
    shortCodeCtrl = self.FindWindowById(NEWENVIRONMENT_TEXTSHORTCODE_ID)
    valueCtrl = self.FindWindowById(NEWENVIRONMENT_TEXTDESCRIPTION_ID)
