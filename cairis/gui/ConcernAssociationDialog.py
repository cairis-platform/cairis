...
class ConcernAssociationDialog(wx.Dialog):
  def __init__(self,parent,dp,envName,source='',sourceMultiplicity='',link='',target='',targetMultiplicity=''):
    ...
    mTypes = ['1','*','1..*']
    mainSizer.Add(WidgetFactory.buildComboSizerList(self,'n',(87,30),CONCERNASSOCIATION_COMBOSOURCEMULTIPLICITY_ID,mTypes),0,wx.EXPAND)
    mainSizer.Add(WidgetFactory.buildComboSizerList(self,'Source',(87,30),CONCERNASSOCIATION_COMBOSOURCE_ID,assets),0,wx.EXPAND)
    mainSizer.Add(WidgetFactory.buildTextSizer(self,'Link',(87,60),CONCERNASSOCIATION_TEXTLINK_ID),0,wx.EXPAND,1,wx.EXPAND)
    mainSizer.Add(WidgetFactory.buildComboSizerList(self,'n',(87,30),CONCERNASSOCIATION_COMBOTARGETMULTIPLICITY_ID,mTypes),0,wx.EXPAND)
    mainSizer.Add(WidgetFactory.buildComboSizerList(self,'Target',(87,30),CONCERNASSOCIATION_COMBOTARGET_ID,assets),0,wx.EXPAND)
    mainSizer.Add(wx.StaticText(self,-1),1,wx.EXPAND)
    mainSizer.Add(WidgetFactory.buildAddCancelButtonSizer(self,CONCERNASSOCIATION_BUTTONCOMMIT_ID),0,wx.ALIGN_CENTER)
    self.SetSizer(mainSizer)

    wx.EVT_BUTTON(self,CONCERNASSOCIATION_BUTTONCOMMIT_ID,self.onCommit)
    self.commitLabel = 'Add'
    if (len(self.theSource) > 0):
      self.commitLabel = 'Edit'
      self.SetLabel('Edit Concern Association')
      smCtrl = self.FindWindowById(CONCERNASSOCIATION_COMBOSOURCEMULTIPLICITY_ID)
      ...
      lCtrl = self.FindWindowById(CONCERNASSOCIATION_TEXTLINK_ID)
      lCtrl.SetValue(self.theLink)
      tmCtrl = self.FindWindowById(CONCERNASSOCIATION_COMBOTARGETMULTIPLICITY_ID)
      tmCtrl.SetStringSelection(self.theTargetMultiplicity)
      tCtrl = self.FindWindowById(CONCERNASSOCIATION_COMBOTARGET_ID)
      tCtrl.SetStringSelection(self.theTarget)
      buttonCtrl = self.FindWindowById(CONCERNASSOCIATION_BUTTONCOMMIT_ID)
  ...
  def onCommit(self,evt):
    ...
    if (len(self.theSource) == 0):
      ...
      self.EndModal(CONCERNASSOCIATION_BUTTONCOMMIT_ID)
