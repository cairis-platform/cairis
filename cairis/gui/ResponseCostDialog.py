...
class ResponseCostDialog(wx.Dialog):
  ...
  def onAdd(self,evt):
    responseCtrl = self.FindWindowById(RESPONSECOST_COMBORESPONSE_ID)
    costCtrl = self.FindWindowById(RESPONSECOST_COMBOCOST_ID)
    ...
    if len(self.theResponseName) == 0:
      ...
      self.EndModal(RESPONSECOST_BUTTONADD_ID)
