...
class StepGrid(wx.grid.Grid):
  def __init__(self,parent,envName):
    wx.grid.Grid.__init__(self,parent,USECASE_GRIDFLOW_ID,wx.DefaultPosition,wx.Size(300,200))
    b = Borg()
    self.dbProxy = b.dbProxy
    self.theEnvironmentName = envName
    self.theUseCase = ''
    self.theMenu = wx.Menu()
    self.theMenu.Append(STEPGRID_MENUDELETE_ID,'Delete')
    self.theMenu.Append(STEPGRID_MENUGOAL_ID,'Refining Goal')
    self.theMenu.Append(STEPGRID_MENUOBSTACLE_ID,'Refining Obstacle')
    self.theMenu.Append(STEPGRID_MENUREQUIREMENT_ID,'Refining Requirement')
    self.theMenu.Append(STEPGRID_MENUSYNOPSIS_ID,'Synopsis')
    self.theMenu.Append(STEPGRID_MENUTAGS_ID,'Tags')
    ...
    self.setTable()
    wx.EVT_MENU(self,STEPGRID_MENUDELETE_ID,self.onDeleteStep)
    wx.EVT_MENU(self,STEPGRID_MENUGOAL_ID,self.onGoal)
    wx.EVT_MENU(self,STEPGRID_MENUOBSTACLE_ID,self.onObstacle)
    wx.EVT_MENU(self,STEPGRID_MENUREQUIREMENT_ID,self.onRequirement)
    ...
    self.obsItem = self.theMenu.FindItemById(STEPGRID_MENUOBSTACLE_ID)
    self.obsItem.Enable(False)
    self.synItem = self.theMenu.FindItemById(STEPGRID_MENUSYNOPSIS_ID)
    self.synItem.Enable(False)
    self.tagsItem = self.theMenu.FindItemById(STEPGRID_MENUTAGS_ID)
  ...
  def onSynopsis(self,evt):
    ...
    if (dlg.ShowModal() == STEPSYNOPSIS_BUTTONCOMMIT_ID):
