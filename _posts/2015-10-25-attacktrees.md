...
![attackTreeSketch]({{ site.baseurl }}/assets/Exploit_vsftpd_backdoor_sketch.pdf)

We can quickly render this tree into something machine readable using [graphviz](http://www.graphviz.org).  Here is the attack tree rendered in graphviz's [Dot language](http://www.graphviz.org/content/dot-language) (downloadable from [here]({{ site.baseurl }}/assets/Exploit_vsftpd_backdoor_graphviz.dot)).
...
![attackTreeRendered]({{ site.baseurl }}/assets/Exploit_vsftpd_backdoor_png.png)
...
```
 ...
 [This file]({{ site.baseurl }}/assets/Exploit_vsftpd_backdoor.xml), which can be imported directly into CAIRIS using either the GUI, or the [cimport](https://github.com/failys/CAIRIS/blob/master/cairis/cairis/cimport.py) model import script.
...
With the model now imported into CAIRIS, it's possible to visualise the model, and start integrating insights from the model into the rest of a system's design.
 ...
 ![atObsModel]({{ site.baseurl }}/assets/Exploit_vsftpd_backdoor_obstacleModel.png)
