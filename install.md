...
* From the web browser of your choice, go the access URL provided by Docker, e.g. http://localhost:32769
...
You can now point your browser to http://SERVERNAME:PORT_NUMBER, depending on where `cairisd` is installed, and what port it is listened to, e.g. http://myserver.org:7071.
...
* Run the CAIRIS database initialisation script `configure_cairis_db.py` in cairis/cairis/bin.  You should be presented with the below form.
...
Open a terminal window and run to `cairis_gui.py` (found in cairis/cairis/bin).
...
Model files can also be imported from the command line by using `cimport.py` (found in cairis/cairis/bin).     
